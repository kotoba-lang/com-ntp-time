(ns ntp_time.main
  "Kotodama WASM entrypoint for the NTP time clean-room actor (L4) — Clojure port.
  ...
  facts (`ntp_time.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ntp_time")
(def tier "L4")
...
(def entity-specs
  [{:entity "Packet"   :plural "packets"   :id-prefix "ntptime_pac"
    :fields [:leapIndicator :version :mode :stratum :poll :precision :rootDelay :rootDispersion :referenceId :referenceTimestamp :originTimestamp :receiveTimestamp :transmitTimestamp]
    :required [:leapIndicator :version]
    :coerce {:leapIndicator :int :version :int :mode :int :stratum :int :poll :int :precision :int :rootDelay :float :rootDispersion :float}
    :refs {}}
   {:entity "Header"   :plural "headers"   :id-prefix "ntptime_hea"
    :fields [:leapIndicator :version :mode :stratum :poll :precision]
    :required [:leapIndicator :version]
    :coerce {:leapIndicator :int :version :int :mode :int :stratum :int :poll :int :precision :int}
    :refs {}}
   {:entity "KissCode" :plural "kisscodes" :id-prefix "ntptime_kis"
    :fields [:code :meaning]
...
(defn emit-facts
  "EAVT facts for one record: {\"ntp_time.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ntp_time-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ntp_time-compat/ready)
